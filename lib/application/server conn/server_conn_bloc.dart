...
class ServerConnBloc extends Bloc<ServerConnEvent, ServerConnState> {
  ServerConnBloc() : super(ServerConnState.initial()) {
    on<Passcode>((event, emit) async {
      ...
      emit(state.copyWith(isLoading: true, passcode: false, passcodeErrorMsg: null));

      try {
        final response = await http.get(Uri.parse('https://www.ballast.in/passcode.txt'));
        ...
        if (response.statusCode == 200) {
          if (event.passcodepin == response.body.trim()) {
            ...
            emit(state.copyWith(isLoading: false, passcode: false, passcodeErrorMsg: 'Incorrect Passcode'));
          }
        } else {
          log('Failed to load passcode data');
          emit(state.copyWith(isLoading: false, passcode: false, passcodeErrorMsg: 'Failed to load passcode'));
        }
      } catch (e) {
        log('Error fetching passcode: $e');
        emit(state.copyWith(isLoading: false, passcode: false, passcodeErrorMsg: 'Error loading passcode data'));
      }
    });

    on<Conn>((event, emit) async {
      log('Connection event initiated');
      emit(state.copyWith(isLoading: true, conn: false, connErrorMsg: null));
      ...
      try {
        ...
        if (isConnected) {
          ...
          emit(state.copyWith(isLoading: false, conn: false, connErrorMsg: 'Failed to connect to server'));
        }
      } catch (e) {
        log('Error during connection attempt: $e');
        emit(state.copyWith(isLoading: false, conn: false, connErrorMsg: 'Connection error: $e'));
      }
    });
  }
}
